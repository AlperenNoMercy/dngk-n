...
module.exports.operate = ({client, author, msg, args, db}) => {
  ...
  if (
    afkSebebi.toLowerCase().includes(".me") ||
    afkSebebi.toLowerCase().includes(".tr") ||
    afkSebebi.toLowerCase().includes(".org") ||
    afkSebebi.toLowerCase().includes(".xyz") ||
  ...
  msg.reply(`**${afkSebebi}** sebebi ile [AFK] moduna giriş yapıldı.`).then(x => x.delete({ timeout: 6000 }));
