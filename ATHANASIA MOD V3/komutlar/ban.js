...
module.exports.operate = async ({msg, client, uye, author, args, db, cfg}) => {
  ...
  if (author.roles.highest.position <= uye.roles.highest.position) return msg.channel.send("**Belirttiğin kişi senden üstte veya onunla aynı yetkidesin!**").then(x => x.delete({timeout: 5000}));
  ...
  await msg.channel.send({embed:{description:`**${uye.user.tag}** adlı üye ${msg.author} adlı yetkili tarafından **${reason}** sebebiyle sunucudan yasaklandı`, color:client.favoriRenkler[Math.floor(Math.random() * client.favoriRenkler.length)], timestamp: new Date()}}).catch();
