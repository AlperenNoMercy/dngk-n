module.exports.operate = async ({client, msg, args, author, uye, cfg}) => {
  ...
  if (!uye) return msg.channel.send("**Bir kişi etiketlemelisin.**").then(m => m.delete({ timeout: 3000 }));
  if (!uye.voice.channel) return msg.channel.send("**Etiketlediğin kişi bir ses kanalında bulunmuyor.**").then(m => m.delete({ timeout: 3000 }));
  if (!args[1]) {
    if (!author.voice.channel) return msg.channel.send("**Bir sesli kanalda bulunmuyorsun.**").then(m => m.delete({ timeout: 4000 }));
    if (author.voice.channel.id === uye.voice.channel.id) return msg.channel.send("**Bu kişiyle zaten aynı kanaldasınız.**").then(m => m.delete({ timeout: 5000 }));
    await uye.voice.setChannel(author.voice.channel.id).catch(err => msg.channel.send(err.message));
    await msg.channel.send(client.nrmlembed(`**${uye} adlı kişi başarıyla kanala çekildi.**`)).then(m => m.delete({ timeout: 5000 }));
  } else if (
    [
      "at",
      "null",
      ...
      await msg.channel.send(client.nrmlembed(`**${uye} adlı kişi başarıyla kanaldan atıldı**`)).then(a => a.delete({ timeout: 5000 }));
    } else {
